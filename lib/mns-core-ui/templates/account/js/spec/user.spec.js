...
describe.only('account : user', () => {
    ...
    describe.only('Process logged in elements', () => {
        it('when logged in elements to show should be displayed and elements to hide should be hidden', () => {
            ...
            documentMock.getElementsByClassName = sinon.stub();
            documentMock.getElementsByClassName.onCall(0).returns(mockElementShowList);
            documentMock.getElementsByClassName.onCall(1).returns(mockElementHideList);
